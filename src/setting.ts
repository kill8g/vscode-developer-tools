...
// 单组配置接口
interface SvnPathPair {
    id: string;
    cs_common_svn_path: string;
    server_svn_path: string;
}
...
class EnvManager {
    ...
    // 保存配置到环境变量 - Base64 版本
    async saveToEnvironmentVariables(config: PluginConfig): Promise<boolean> {
        try {
            // 1. 将配置转换为 JSON 字符串
    ...
    // 设置单个环境变量
    private async setEnvVar(key: string, value: string): Promise<void> {
        return new Promise((resolve, reject) => {
            try {
                ...
                regKey.set(key, winreg.REG_SZ, value, (err: any) => {
                    if (err) {
    ...
    private async readRegistryAsync(regKey: any, valueName: string): Promise<string | undefined> {
        return new Promise((resolve, reject) => {
            try {
    ...
    async loadFromEnvironmentVariables(): Promise<PluginConfig> {
        try {
    ...
    // 清除环境变量
    async clearEnvironmentVariables(): Promise<boolean> {
        try {
            // 删除主环境变量
    ...
    // 删除单个环境变量
    private async deleteEnvVar(key: string): Promise<void> {
        return new Promise((resolve, reject) => {
            try {
                const winreg = require('winreg');
                const regKey = new winreg({
...
// 主函数
export async function ModifySetting(context: vscode.ExtensionContext) {
    try {
        ...
        const panel = vscode.window.createWebviewPanel(
            'wDeveloperToolsConfig',
            'W Developer Tools 配置',
                ...
                switch (message.command) {
                    case 'saveConfig':
                        try {
                            ...
                            if (configData.projects) {
                                configData.projects.forEach((project: ProjectConfig) => {
                    ...
                    case 'requestClearConfirm':
                        ...
                        const confirmResult = await vscode.window.showWarningMessage(
                            '确定要清除所有配置吗？此操作会删除环境变量中的配置，不可撤销。',
                            { modal: true },
                            '确定清除',
                            '取消'
                        );
...
// 获取 WebView HTML 内容
function getWebviewContent(webview: vscode.Webview, extensionUri: vscode.Uri, currentConfig: PluginConfig): string {
    // 序列化整个配置
